...
public class SerialComm implements SerialPortEventListener {
	...
	public void open(CommPortIdentifier portId) {
		synchronized(this) {
			...
			try {
				...
				Application.println(String.format("Opened serial port: %s",
						serialPort.getName()));
			} catch(PortInUseException e) {
				Application.printerr(String.format("Port in use: %s",
	...
	public void close() {
		synchronized(this) {
			if(serialPort != null) {
				...
				Application.println(String.format("Closed serial port: %s",
						serialPortName));
			}
		}
	}
	...
	public void sendCommand(String cmd) {
		try {
			...
			Application.printerr("sendCommand: "
					+ String.format("Cannot send command '%s' (%s)", cmd, e));
		}
	}
			...
			try {
				while((data = inputStream.read()) > -1) {
					...
					if(data == '\n' || data == '\r') {
						...
						Application.getApplication().getDataProcessor()
								.parseAnswerAndNotify(received);
		...
		new Thread() {
			...
			public void run() {
				while(bPeriodic) {
					...
					sendCommand(Application.getApplication().getConfig()
							.getPeriodicReadCommand());
					try {
						Thread.sleep(Application.getApplication().getConfig()
								.getPeriodicReadInterval());
